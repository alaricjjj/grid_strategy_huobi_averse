    ...
    def run(self):
        ...
        while True:
            ...
            # 如果仓位大于0，则判断是否出发追踪止损
                ...
                if self.current_hour != self.last_hour:
                    message = 'Time: %s, average cost is: %s, current price is: %s, stoptrail triger point is: %s, stoptrail_trail_ing is: %s. ' \
                              % (time.strftime("%Y-%m-%d %H:%M:%S"),avg_cost,self.new_price, avg_cost * self.stoptrail_triger_percent,self.stop_trial_ing)
                    self.dingding_notice(message)
                    print(message)
