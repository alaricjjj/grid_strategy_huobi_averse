...
class Grid_long_stoptrail():
        ...
        while True:
            ...
            if position_check == False:
                ...
                if self.current_target_coin_trade > 0.000002:
                    ...
                    message = 'Initial target coin position is not 0, clean position, trade amount is %s' % (
                        trade_amount)
                    print(message)
                    # aa = self.huobi_spot_client.create_order(symbol=trade_symbol, type='market', side='sell',amount=self.format_amount(trade_amount))
                    aa = self.huobi_spot_client.create_order(symbol=trade_symbol, type='sell-market',
                                                             amount=str(self.format_amount(trade_amount)))
                    position_check = True
    ...
    def palce_orders(self):
        ...
        # 计算每份交易使用的仓位
        first_amount = self.current_base_coin_trade / 259
        ...
        grid_price = [self.current_price * 0.999363958,self.current_price * 0.994770318,self.current_price * 0.978869258,self.current_price * 0.932508834,
                      self.current_price * 0.889964664,self.current_price * 0.875759717,self.current_price * 0.856537102,self.current_price * 0.836042403,
                      self.current_price * 0.814204947,self.current_price * 0.794204947,self.current_price * 0.763214542]
        # 先下一个市价单买入最小仓位
        ...
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[2])),amount=str(self.format_amount(first_amount/grid_price[2] * 1)))
        ...
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[4])),amount=str(self.format_amount(first_amount/grid_price[4] * 2)))
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[5])),amount=str(self.format_amount(first_amount/grid_price[5] * 4)))
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[6])),amount=str(self.format_amount(first_amount/grid_price[6] * 8)))
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[7])),amount=str(self.format_amount(first_amount/grid_price[7] * 16)))
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[8])),amount=str(self.format_amount(first_amount/grid_price[8] * 32)))
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[9])),amount=str(self.format_amount(first_amount/grid_price[9] * 64)))
        self.huobi_spot_client.create_order(symbol=trade_symbol,type='buy-limit',price=str(self.format_price(grid_price[10])),amount=str(self.format_amount(first_amount/grid_price[10] * 126)))
        ...
        while True:
            ...
            # 如果仓位大于0，则判断是否出发追踪止损
                ...
                if position_ratio < 0.01:
                    self.stoptrail_triger_percent = 1.0332
                elif position_ratio < 0.05:
                    self.stoptrail_triger_percent = 1.031
                elif position_ratio < 0.1:
                    self.stoptrail_triger_percent = 1.0276
                elif position_ratio < 0.15:
                    self.stoptrail_triger_percent = 1.0224
                else:
                    self.stoptrail_triger_percent = 1.0168
